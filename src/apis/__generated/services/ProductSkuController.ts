...
export class ProductSkuController {
  ...
  async findById(
    ...
    return (await this.executor({
      uri: _uri,
      method: "GET",
    })) as ProductSkuDto["ProductSkuRepository/COMPLEX_FETCHER"];
  }
  ...
  async query(
    ...
    return (await this.executor({
      ...
      method: "POST",
      body: options.body,
    })) as Page<ProductSkuDto["ProductSkuRepository/COMPLEX_FETCHER"]>;
  }
...
export type ProductSkuControllerOptions = {
  ...
  findById: { id: string };
  query: { body: QueryRequest<ProductSkuSpec> };
  save: { body: ProductSkuInput };
};
